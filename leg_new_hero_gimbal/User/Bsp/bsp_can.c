...
uint8_t CAN_DM_imu_Tx_Data[8];

//双板通信发送缓冲区
uint8_t CAN3_0xA1_Chassis_Control_Tx_Data[8];
uint8_t CAN3_0xA2_DT7_1_Tx_Data[8];
uint8_t CAN3_0xA3_DT7_2_Tx_Data[8];
uint8_t CAN3_0xA4_VT13_1_Tx_Data[8];
uint8_t CAN3_0xA5_VT13_2_Tx_Data[8];
uint8_t CAN3_0xA6_Fusi_1_Tx_Data[8];
uint8_t CAN3_0xA7_Fusi_2_Tx_Data[8];
...
void CAN_Init(FDCAN_HandleTypeDef* hfdcan, CAN_Call_Back Callback_Function)
{
    ...
    else if (hfdcan->Instance == FDCAN2)
    {
        ...
        // 修改全局过滤器配置 - 接受所有不匹配的帧到FIFO0
        // HAL_FDCAN_ConfigGlobalFilter(&hfdcan2, FDCAN_ACCEPT_IN_RX_FIFO0, // 不匹配的标准帧到FIFO0
        //                              FDCAN_ACCEPT_IN_RX_FIFO0, // 不匹配的扩展帧到FIFO0
        ...
        HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_RX_FIFO0_NEW_MESSAGE | // FIFO0新消息中断
    ...
    else if (hfdcan->Instance == FDCAN3)
    {
        ...
        // 修改全局过滤器配置 - 接受所有不匹配的帧到FIFO0
        // HAL_FDCAN_ConfigGlobalFilter(&hfdcan3, FDCAN_ACCEPT_IN_RX_FIFO0, // 不匹配的标准帧到FIFO0
        //                              FDCAN_ACCEPT_IN_RX_FIFO0, // 不匹配的扩展帧到FIFO0
        //                              FDCAN_REJECT_REMOTE, // 拒绝远程帧
        //                              FDCAN_REJECT_REMOTE); // 拒绝远程帧
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs)
{
    ...
    if (hfdcan->Instance == FDCAN1)
    {
        HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &CAN1_Manage_Object.Rx_Buffer.Header,
    ...
    else if (hfdcan->Instance == FDCAN2)
    {
        ...
        if (HAL_FDCAN_GetRxMessage(hfdcan,FDCAN_RX_FIFO0, &rx_header, CAN2_Manage_Object.Rx_Buffer.Data) == HAL_OK)
        {
            CAN2_Manage_Object.Rx_Buffer.Header.Identifier = rx_header.Identifier;
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs)
{
    ...
    if (hfdcan->Instance == FDCAN1)
    {
        HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO1, &CAN1_Manage_Object.Rx_Buffer.Header,
                               CAN1_Manage_Object.Rx_Buffer.Data);
        CAN1_Manage_Object.Callback_Function(&CAN1_Manage_Object.Rx_Buffer);
    }
    else if (hfdcan->Instance == FDCAN2)
    {
        ...
        if (HAL_FDCAN_GetRxMessage(hfdcan,FDCAN_RX_FIFO1, &rx_header, CAN2_Manage_Object.Rx_Buffer.Data) == HAL_OK)
        {
            CAN2_Manage_Object.Rx_Buffer.Header.Identifier = rx_header.Identifier;
            if (rx_header.DataLength <= FDCAN_DLC_BYTES_8) len = rx_header.DataLength;
            else if (rx_header.DataLength == FDCAN_DLC_BYTES_12) len = 12;
