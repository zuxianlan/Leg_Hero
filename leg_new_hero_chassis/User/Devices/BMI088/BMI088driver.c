...
void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
    ...
    do
    {
        ...
        for (uint16_t i = 0; i < CaliTimes; i++)
        {
            ...
            gNormTemp = sqrtf(
                bmi088->Accel[0] * bmi088->Accel[0] + bmi088->Accel[1] * bmi088->Accel[1] + bmi088->Accel[2] * bmi088->
                Accel[2]);
    ...
    while (gNormDiff > 0.5f || fabsf(bmi088->gNorm - 9.8f) > 0.5f || gyroDiff[0] > 0.15f || gyroDiff[1] > 0.15f ||
           gyroDiff[2] > 0.15f || fabsf(bmi088->GyroOffset[0]) > 0.01f || fabsf(bmi088->GyroOffset[1]) > 0.01f || fabsf(
               bmi088->GyroOffset[2]) > 0.01f);
