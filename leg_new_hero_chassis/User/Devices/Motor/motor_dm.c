 ...
 */
void Motor_DM_Normal_Init(Motor_DM_Normal *motor, FDCAN_HandleTypeDef *hcan, uint8_t __CAN_Rx_ID, uint8_t __CAN_Tx_ID,
                          Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, float __Angle_Max, float __Omega_Max,
...
void Motor_DM_Normal_Data_Process(Motor_DM_Normal *motor)
{
    ...
    Struct_Motor_DM_CAN_Rx_Data_Normal *tmp_buffer = (Struct_Motor_DM_CAN_Rx_Data_Normal *) motor->CAN_Manage_Object->
            Rx_Buffer.Data;
    ...
    // 计算电机本身信息
    motor->Rx_Data.Now_Angle = (float) (motor->Rx_Data.Total_Encoder) / (float) ((1 << 16) - 1) * motor->Angle_Max *
                               2.0f;
...
void Motor_DM_Normal_Output(Motor_DM_Normal *motor)
{
    ...
    case Motor_DM_Control_Method_NORMAL_ANGLE_OMEGA:
    {
        Struct_Motor_DM_CAN_Tx_Data_Normal_Angle_Omega *tmp_buffer = (Struct_Motor_DM_CAN_Tx_Data_Normal_Angle_Omega *)
                motor->Tx_Data;
    ...
    case Motor_DM_Control_Method_NORMAL_OMEGA:
    {
        Struct_Motor_DM_CAN_Tx_Data_Normal_Omega *tmp_buffer = (Struct_Motor_DM_CAN_Tx_Data_Normal_Omega *) motor->
                Tx_Data;
    ...
    case Motor_DM_Control_Method_NORMAL_EMIT:
    {
        Struct_Motor_DM_CAN_Tx_Data_Normal_EMIT *tmp_buffer = (Struct_Motor_DM_CAN_Tx_Data_Normal_EMIT *) motor->
                Tx_Data;
 ...
 */
void Motor_DM_1_To_4_Init(Motor_DM_1_To_4 *motor, FDCAN_HandleTypeDef *hcan, Enum_Motor_DM_Motor_ID_1_To_4 __CAN_Rx_ID,
                          Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, int32_t __Encoder_Offset,
                          float __Current_Max)
{
    if (hcan->Instance == FDCAN1)
    {
    ...
    motor->RC = get_remote_control_point();
}
...
void Motor_DM_1_To_4_Data_Process(Motor_DM_1_To_4 *motor)
{
    ...
    int16_t tmp_omega = 0, tmp_current = 0;
    uint16_t offset = 0;
    Struct_Motor_DM_CAN_Rx_Data_1_To_4 *tmp_buffer = (Struct_Motor_DM_CAN_Rx_Data_1_To_4 *) motor->CAN_Manage_Object->
            Rx_Buffer.Data;
    ...
    motor->Rx_Data.Total_Encoder = motor->Rx_Data.Total_Round * motor->Encoder_Num_Per_Round + tmp_encoder + motor->
                                   Encoder_Offset;

    // 计算电机本身信息
    motor->Rx_Data.Now_Angle = (float) motor->Rx_Data.Total_Encoder / (float) motor->Encoder_Num_Per_Round * 2.0f * PI;
    if (motor->RC->RC.rotory_sw[1] >= 0.5f)
    {
        offset = 500;
    }
    else
    ...
    // motor->Rx_Data.Now_Angle = motor_ecd_to_angle_change(tmp_encoder, offset);
...
void Motor_DM_1_To_4_PID_Calculate(Motor_DM_1_To_4 *motor)
{
    ...
    case (Motor_DM_Control_Method_1_TO_4_GYRO_ANGLE):
    {
        //YAW轴绝对角控制，PITCH不在内
        motor->Angle_PID.Target = motor->Target_Angle;
        ...
        // motor->Angle_PID.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Omega_PID);
