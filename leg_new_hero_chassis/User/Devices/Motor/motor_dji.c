 ...
 */
uint8_t *allocate_tx_data_dji(FDCAN_HandleTypeDef *hcan, Enum_CAN_Motor_ID __CAN_ID,
                              Enum_Motor_DJI_GM6020_Driver_Version __DJI_Motor_Driver_Version)
{
...
void Motor_GM6020_Init(Motor_GM6020 *motor, FDCAN_HandleTypeDef *hcan, Enum_CAN_Motor_ID __CAN_ID,
                       Enum_Control_Method __Control_Method, int32_t __Encoder_Offset,
                       Enum_Motor_DJI_Power_Limit_Status __Power_Limit_Status,
                       Enum_Motor_DJI_GM6020_Driver_Version __Driver_Version, float __Voltage_Max, float __Current_Max)
{
...
void Motor_GM6020_CAN_RxCpltCallback(Motor_GM6020 *motor, uint8_t *Rx_Data)
{
    ...
    Struct_Motor_DJI_CAN_Rx_Data *tmp_buffer = (Struct_Motor_DJI_CAN_Rx_Data *) motor->CAN_Manage_Object->Rx_Buffer.
            Data;
    ...
    motor->Rx_Data.Now_Power = power_calculate(motor->Power_K_0, motor->Power_K_1, motor->Power_K_2, motor->Power_A,
                                               motor->Rx_Data.Now_Current, motor->Rx_Data.Now_Omega);
...
{
    // 计算功率估计值
    motor->Power_Estimate = power_calculate(motor->Power_K_0, motor->Power_K_1, motor->Power_K_2, motor->Power_A,
                                            motor->Target_Current, motor->Rx_Data.Now_Omega);
    ...
    if (motor->Power_Estimate > 0.0f)
    {
        ...
        else
        {
            ...
            float c = motor->Power_A + motor->Power_K_1 * motor->Rx_Data.Now_Omega * motor->Rx_Data.Now_Omega - motor->
                      Power_Factor * motor->Power_Estimate;
            float delta, h;
            delta = b * b - 4 * a * c;
            if (delta < 0.0f)
            {
            ...
            else
            {
                ...
                {
                    if ((motor->Target_Current > 0.0f && result_1 > 0.0f) || (
                            motor->Target_Current < 0.0f && result_1 < 0.0f))
                    {
...
void Motor_C610_Init(Motor_C610 *motor, FDCAN_HandleTypeDef *hcan, Enum_CAN_Motor_ID __CAN_ID,
...
void Motor_C620_Init(Motor_C620 *motor, FDCAN_HandleTypeDef *hcan, Enum_CAN_Motor_ID __CAN_ID,
                     Enum_Control_Method __Control_Method, Enum_Motor_DJI_Power_Limit_Status __Power_Limit_Status,
                     float __Current_Max)
{
    if (hcan->Instance == FDCAN1)
    {
