%% 云台Pitch轴跟踪LQR 全自动生成整套代码
clear; clc; close all;

% =====================【唯一手动修改区】=====================
Q = diag([770, 0.9]);   % [角度误差权重q1, 角速度权重q2]
R = 0.7;              % 控制力矩权重
J = 0.02;             % Pitch轴转动惯量 kg·m²
B = 0.04;              % 粘性阻尼 N·m·s/rad
u_max = 1.0;           % 最大输出力矩限幅
u_min = -1.0;
% ==========================================================

%% 1. 构建连续状态空间（和Yaw结构完全一致）
A = [0,      1;
     0, -B/J];
Bm = [0; 1/J];
C  = [1, 0];
...
%% 3. 自动覆写Pitch_LQR_Calc.m函数，填充最新K1、K2
fileID = fopen('Pitch_LQR_Calc.m','w');
fprintf(fileID,'function u_torque = Pitch_LQR_Calc(theta_ref, theta_now, omega_now)\n');
fprintf(fileID,'%%#codegen\n');
fprintf(fileID,'%% Pitch轴自动生成LQR增益，无需手动修改\n');
...
fprintf(fileID,'end\n');
fclose(fileID);
disp('已自动更新 Pitch_LQR_Calc.m 内K1/K2数值');
