...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    //云台yaw轴初始化正对角度
    init->init_angle=2.34f;

    //设置pitch轴绝对角最大值
    init->gimbal_pitch_j4340.max_absolute_angle = 0.55f;
    init->gimbal_pitch_j4340.min_absolute_angle = -0.3f;
...
// pitch 实际目标被限幅削掉（撞墙/切模式/一键吊射）时，把反弹累加器拉回来对齐，
// 避免"死行程"：撞墙后下一次拨动立即反向
void Pitch_Target_Sync(gimbal_control_t *set_control)
{
    if (set_control->gimbal_pitch_j4340.dm_normal_motor.Target_Angle !=
        pitch_target)
    {
        pitch_target = set_control->gimbal_pitch_j4340.dm_normal_motor.Target_Angle;
    }
}
/**
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    gimbal_behaviour_control_set(&add_yaw_angle, &add_pitch_angle, set_control);
     ...
     else if (set_control->gimbal_pitch_j4340.gimbal_motor_mode == GIMBAL_MOTOR_ABSOLUTE_ANGLE_CONTROL)
     {
         ...
         Pitch_Target_Sync(set_control);   // 新增
     }
