...
float dm_motor_normalize_angle(float angle)
{
 ...
 */
void Motor_DM_Normal_Init(Motor_DM_Normal *motor, FDCAN_HandleTypeDef *hcan, uint8_t __CAN_Rx_ID, uint8_t __CAN_Tx_ID, Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, float __Angle_Max, float __Omega_Max, float __Torque_Max, float __Current_Max, int8_t polarity)
{
...
void Motor_DM_Normal_Control_Mode_Transit_PeriodElapsedCallback(Motor_DM_Normal *motor,Enum_Motor_DM_Control_Method mode)
{
    motor->Motor_DM_Control_Method=mode;
}
...
void Motor_DM_Normal_TIM_Alive_PeriodElapsedCallback(Motor_DM_Normal *motor)
{
    ...
    if (motor->Flag == motor->Pre_Flag)
    {
        ...
        // //清除积分项并发送0力矩
        // PID_Set_Integral_Error(&motor->Angle_PID, 0.0f);
        // PID_Set_Integral_Error(&motor->Omega_PID, 0.0f);
        // PID_Set_Integral_Error(&motor->Only_Omega_PID, 0.0f);
        // motor->Control_Torque=0;
    }
    else
    {
        // 电机保持连接
...
void Motor_DM_Normal_Zero_Force_PeriodElapsedCallback(Motor_DM_Normal *motor)
{
    //清除积分项并发送0力矩
    PID_Set_Integral_Error(&motor->Angle_PID, 0.0f);
    PID_Set_Integral_Error(&motor->Omega_PID, 0.0f);
...
void Trigger_Motor_DM_Normal_TIM_1ms_Calculate_PeriodElapsedCallback(Motor_DM_Normal *motor)
{

    if (motor->motor_pid_control_mode==angle_pid_control)
    {
        ...
        motor->Omega_PID.Target = motor->Target_Omega;
        motor->Omega_PID.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Omega_PID);

        motor->Control_Torque = (motor->Omega_PID.Out + motor->Feedforward_Torque) * motor->polarity;
    }
    else if (motor->motor_pid_control_mode==only_omega_pid_control)
    {
        motor->Only_Omega_PID.Target = motor->Target_Omega;
        motor->Only_Omega_PID.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Only_Omega_PID);

        motor->Control_Torque = motor->Only_Omega_PID.Out * motor->polarity;
    }
    else
    {
        motor->Control_Torque = 0;
...
void Yaw_Motor_DM_Normal_TIM_1ms_Calculate_PeriodElapsedCallback(Motor_DM_Normal *motor)
{
    motor->Angle_PID.Target = motor->Target_Angle;
    motor->Angle_PID.Now = motor->INS_point->Yaw;
    PID_TIM_Adjust_PeriodElapsedCallback(&motor->Angle_PID);

    motor->Target_Omega = motor->Angle_PID.Out;

    motor->Omega_PID.Target = motor->Target_Omega+motor->Feedforward_Omega;
    motor->Omega_PID.Now = motor->INS_point->Gyro[2];
    PID_TIM_Adjust_PeriodElapsedCallback(&motor->Omega_PID);

    motor->Control_Torque = motor->Omega_PID.Out * motor->polarity;
}

void Pitch_Motor_DM_Normal_TIM_1ms_Calculate_PeriodElapsedCallback(Motor_DM_Normal *motor)
{
    motor->Angle_PID.Target = motor->Target_Angle;
    motor->Angle_PID.Now = -motor->INS_point->Pitch;
    PID_TIM_Adjust_PeriodElapsedCallback(&motor->Angle_PID);
    ...
    motor->Control_Torque = motor->Omega_PID.Out * motor->polarity + motor->Feedforward_Torque + 0.1f*arm_cos_f32( -motor->INS_point->Gyro[1]);
}
 ...
 */
void Motor_DM_1_To_4_Init(Motor_DM_1_To_4 *motor, FDCAN_HandleTypeDef *hcan, Enum_Motor_DM_Motor_ID_1_To_4 __CAN_Rx_ID, Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, int8_t polarity, int32_t __Encoder_Offset, float __Current_Max)
{
    if (hcan->Instance == FDCAN1)
    {
        motor->CAN_Manage_Object = &CAN1_Manage_Object;
...
void Motor_DM_1_To_4_RC_ctrl_Disable_PeriodElapsedCallback(Motor_DM_1_To_4 *motor)
{
     // 电机断开连接
     motor->Motor_DM_Status = Motor_DM_Status_DISABLE;
     //清除积分项并发送零电流
...
void Motor_DM_1_To_4_Control_Mode_Transit_PeriodElapsedCallback(Motor_DM_1_To_4 *motor,Enum_Motor_DM_Control_Method mode)
{
    motor->Motor_DM_Control_Method=mode;
}
...
void Motor_DM_1_To_4_TIM_1ms_Calculate_PeriodElapsedCallback(Motor_DM_1_To_4 *motor)
{
    ...
    motor->Out = (tmp_value * motor->Current_To_Out) * motor->polarity;
...
void Motor_DM_1_To_4_PID_Calculate(Motor_DM_1_To_4 *motor)
{
    ...
    case (Motor_DM_Control_Method_1_TO_4_CURRENT):
        break;
    case (Motor_DM_Control_Method_1_TO_4_ONLY_OMEGA):
    {
        motor->Only_Omega_PID.Target = motor->Target_Omega + motor->Feedforward_Omega;
        motor->Only_Omega_PID.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Only_Omega_PID);

        motor->Target_Current = motor->Only_Omega_PID.Out;
        break;
    }
    ...
    case (Motor_DM_Control_Method_1_TO_4_RELATIVE_ANGLE_YAW):
    {
        motor->Relative_Angle_PID.Target = motor->Target_Angle;
        motor->Relative_Angle_PID.Now = motor->Rx_Data.Now_Angle;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Relative_Angle_PID);

        motor->Target_Omega = motor->Relative_Angle_PID.Out;

        motor->Relative_Omega_PID.Target = motor->Target_Omega + motor->Feedforward_Omega;
        motor->Relative_Omega_PID.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->Relative_Omega_PID);

        motor->Target_Current = motor->Relative_Omega_PID.Out;

        break;
    }
    case (Motor_DM_Control_Method_1_TO_4_GYRO_ANGLE_YAW):
    {
        //YAW轴绝对角控制
        motor->Angle_PID.Target = motor->Target_Angle;
        motor->Angle_PID.Now = motor->INS_point->Yaw;
    ...
    case (Motor_DM_Control_Method_1_TO_4_GYRO_ANGLE_PITCH):
    {
        //PITCH轴绝对角控制
        motor->Angle_PID.Target = motor->Target_Angle;
        motor->Angle_PID.Now = -motor->INS_point->Pitch;
