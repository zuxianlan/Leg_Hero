...
typedef enum {
    ...
    //CAN1 摩擦轮和二级拨弹
    CAN1_Left_Fric_Motor_ID_0x201=1,
    CAN1_Right_Fric_Motor_ID_0x202=2,
...
typedef enum {
    ...
    Motor_DJI_Control_Method_RPM,
    Motor_DJI_Control_Method_ONLY_OMEGA
} Enum_Control_Method;
...
typedef struct {
    ...
    float Target_RPM;       //目标的转速rpm //单位 r/min
    float Target_Current; 	// 目标的扭矩
    float Feedforward_Omega;    // 前馈的速度, rad/s
    float Feedforward_Current;    // 前馈的电流, A
    float Feedforward_Torque;
    float Accel_Feedforward_Torque;
    float Diff_Feedforward_Torque;
    float Out; 				// 输出量
