...
float dj_motor_normalize_angle(float angle)
{
...
void Motor_GM6020_CAN_RxCpltCallback(Motor_GM6020 *motor, uint8_t *Rx_Data)
{
    ...
    motor->Rx_Origin.Encoder_Reverse = tmp_encoder;
    motor->Rx_Origin.Omega_Reverse = tmp_omega;
    motor->Rx_Origin.Current_Reverse = tmp_current;
...
void Motor_C620_CAN_RxCpltCallback(Motor_C620 *motor, uint8_t *Rx_Data)
{
    ...
    motor->Rx_Data.Now_Acceleration = (motor->Rx_Data.Now_Omega - motor->Rx_Data.Pre_Omega) / 0.001f;
    motor->Rx_Data.Now_Power = power_calculate(motor->Power_K_0, motor->Power_K_1, motor->Power_K_2, motor->Power_A, motor->Rx_Data.Now_Current, motor->Rx_Data.Now_Omega);

    // 存储预备信息
    motor->Rx_Data.Pre_Encoder = tmp_encoder;
    motor->Rx_Data.Pre_Omega = motor->Rx_Data.Now_Omega;
...
void Motor_C620_Zero_Force_PeriodElapsedCallback(Motor_C620 *motor)
{
    //清除积分项并清零输出
    PID_Set_Integral_Error(&motor->PID_Angle,0.0f);
    PID_Set_Integral_Error(&motor->PID_Omega,0.0f);
...
void Motor_C620_RC_ctrl_Disable_PeriodElapsedCallback(Motor_C620 *motor)
{
    // 设置离线状态
    motor->CAN_Motor_Status = CAN_Motor_Status_DISABLE;
    //清除积分项并清零输出
...
void Shoot_Fric_Motor_C620_TIM_Calculate_PeriodElapsedCallback(Motor_C620 *motor)
{
    Motor_C620_TIM_PID_PeriodElapsedCallback(motor);

    float tmp_value = motor->Target_Current + motor->Feedforward_Torque;
    motor->Out = tmp_value;
...
void Motor_C620_TIM_PID_PeriodElapsedCallback(Motor_C620 *motor)
{
    ...
    case Motor_DJI_Control_Method_CURRENT:
        ...
        break;
    case Motor_DJI_Control_Method_ONLY_OMEGA:

        motor->PID_Only_Omega.Target = motor->Target_Omega + motor->Feedforward_Omega;
        motor->PID_Only_Omega.Now = motor->Rx_Data.Now_Omega;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->PID_Only_Omega);

        motor->Target_Current = motor->PID_Only_Omega.Out;
    ...
    case Motor_DJI_Control_Method_ANGLE:
        ...
        motor->Target_Current = motor->PID_Omega.Out;
        break;
    case Motor_DJI_Control_Method_RPM:

        motor->PID_RPM.Target = motor->Target_RPM;
        motor->PID_RPM.Now = motor->Rx_Origin.Omega_Reverse;
        PID_TIM_Adjust_PeriodElapsedCallback(&motor->PID_RPM);

        motor->Target_Current = motor->PID_RPM.Out;
...
void Motor_DJ_C620_Control_Mode_Transit_PeriodElapsedCallback(Motor_C620 *motor,Enum_Control_Method mode)
{
    motor->Control_Method=mode;
}